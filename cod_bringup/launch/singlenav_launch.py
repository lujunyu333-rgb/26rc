...
def generate_launch_description():
    ...
    load_nodes = GroupAction(
        actions=[
            # startup_pose_manager 由 cod.sh / z2.sh 分别启动，支持不同比赛参数
            Node(
            ...
            # 已由 startup_pose_manager 动态管理 map→odom（支持红蓝方切换）
            # Node(
            #     package="tf2_ros",
            #     executable="static_transform_publisher",
            #     name="map_to_odom",
            ...
            #     }.items()
            # ),
            # 先启动 localization（map_server + lifecycle），确保地图先发布
            IncludeLaunchDescription(
            ...
            # 延迟 2 秒再启动 navigation，给 map_server 时间发布第一帧地图
            TimerAction(
                period=2.0,
                actions=[
                    IncludeLaunchDescription(
