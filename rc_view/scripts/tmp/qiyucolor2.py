...
# -*- coding: utf-8 -*-
"""
区域颜色检测节点1 - 从USB摄像头(device=0)采集图像，通过HSV颜色过滤
同时检测黄色和紫色两种目标区域，合并为单一 flag 发布。

flag 含义: 0=无目标, 1=黄色, 2=紫色
"""
...
class QiyuColorNode(Node):
    """同时检测黄色和紫色两种目标颜色，合并为单一 flag 发布"""

    def __init__(self, device_path: int, node_name: str):
        super().__init__(node_name)

        # 单一发布者: 0=无, 1=黄色, 2=紫色
        self.flag_pub = self.create_publisher(UInt8, "area_flag1", 10)
    ...
    def _detect_color(self, hsv_img, hue_min, hue_max, sat_min, sat_max,
        ...
        proportion = area / self.total_pixels
        return (1 if proportion > trigger_ratio else 0, area, mask)

    def _draw_visualization(self, frame, y_mask, p_mask, flag, y_area, p_area):
        """结果图(原图+轮廓+标注) + 掩码图(黄青紫洋红合成)"""
        # 状态映射
        status_map = {0: "NONE", 1: "YELLOW", 2: "PURPLE"}
        status_text = status_map.get(flag, "?")
        flag_color = (
            (0, 255, 0) if flag == 1 else
            (255, 0, 255) if flag == 2 else
            (128, 128, 128)
        )
        ...
        # ── 掩码图：黄=青，紫=洋红，叠加到一张图上 ──
        mask_bgr = np.zeros((frame.shape[0], frame.shape[1], 3), dtype=np.uint8)
        mask_bgr[y_mask > 0] = (0, 255, 255)     # 青色
        mask_bgr[p_mask > 0] = (255, 0, 255)     # 洋红
    ...
    def run(self):
        while rclpy.ok():
            ...
            self.get_logger().info(
                f"黄色: area={y_area}px | 紫色: area={p_area}px | flag={flag}",
                throttle_duration_sec=1.0)

            # 可视化显示
            result, mask_bgr = self._draw_visualization(
                frame, y_mask, p_mask, flag, y_area, p_area)
            cv2.imshow("qiyu1_result", result)
            cv2.imshow("qiyu1_mask", mask_bgr)
...
def main(args=None):
    ...
    try:
        node = QiyuColorNode(device_path=2, node_name="qiyu1_publisher")
        node.run()
    except RuntimeError as e:
        rclpy.logging.get_logger("qiyucolor1").error(str(e))
