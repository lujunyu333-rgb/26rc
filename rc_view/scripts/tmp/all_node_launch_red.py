...
一键启动所有视觉检测节点：
  ...
  - res_kfs         RealSense 红/蓝区域检测（5s 延迟发送）
  - serial_node     串口桥接（auto_serial_bridge 已安装时自动加载）

用法：
  ros2 launch camera all_node_launch_red.py
  ros2 launch camera all_node_launch_red.py target_color:=blue    # 改 res_kfs 检测颜色
  ros2 launch camera all_node_launch_red.py start_yolo:=false     # 跳过 YOLO
  ros2 launch camera all_node_launch_red.py start_rs:=false       # 跳过 RealSense
...
def launch_setup(context, *args, **kwargs):
    ...
    # ── 5. res_kfs — RealSense 红/蓝区域检测（5s 延迟发送）──
    res_kfs_node = Node(
        package='camera',
        executable='res_kfs',
        name='reg_kfs',
    ...
    try:
        ...
        print('[all_node_launch_red] auto_serial_bridge 未安装，跳过串口桥接节点', file=sys.stderr)
        serial_node = None
...
def generate_launch_description():
    ...
    start_res_kfs_arg = DeclareLaunchArgument(
        ...
        description='启动 res_kfs (RealSense 红/蓝区域检测)')
