...
public:
  LidarFilterNode() : Node("lidar_filter_node")
  {
    ...
    // 裁剪参数：按 livox_frame 坐标系裁剪
    // 雷达在车头，高约 30cm：
    // x 负方向为车身后方，主要裁后方 65cm
    // y 左右按车宽裁
    // z 裁雷达下方车身区域
    this->declare_parameter("min_x", -0.65);
    this->declare_parameter("max_x", 0.03);
    this->declare_parameter("min_y", -0.38);
    this->declare_parameter("max_y", 0.33);
    this->declare_parameter("min_z", -0.35);
    this->declare_parameter("max_z", 0.10);
    this->declare_parameter("negative", true); // true = 删除盒子内部点
...
private:
  void publish_marker()
  {
    ...
    // 注意：实际 CropBox 是按输入点云坐标系裁剪的。
    // /livox/lidar/pointcloud 的 frame_id 是 livox_frame，
    // 所以 marker 也必须放在 livox_frame，才能和真实裁剪盒对齐。
