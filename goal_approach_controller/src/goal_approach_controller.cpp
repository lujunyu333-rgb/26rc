...
public:
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) override
  {
    ...
    nav2_util::declare_parameter_if_not_declared(
      node, name + ".direct_rotate_xy_tolerance",
      rclcpp::ParameterValue(0.05));
    nav2_util::declare_parameter_if_not_declared(
      node, name + ".direct_yaw_tolerance",
      rclcpp::ParameterValue(0.10));
    nav2_util::declare_parameter_if_not_declared(
      node, name + ".direct_yaw_kp",
      rclcpp::ParameterValue(2.0));
    nav2_util::declare_parameter_if_not_declared(
      node, name + ".direct_yaw_max_vel",
      rclcpp::ParameterValue(0.8));
    nav2_util::declare_parameter_if_not_declared(
      node, name + ".direct_yaw_min_vel",
    ...
    node->get_parameter(name + ".direct_rotate_xy_tolerance", direct_rotate_xy_tolerance_);
    node->get_parameter(name + ".direct_yaw_tolerance", direct_yaw_tolerance_);
    node->get_parameter(name + ".direct_yaw_kp", direct_yaw_kp_);
    node->get_parameter(name + ".direct_yaw_max_vel", direct_yaw_max_vel_);
    node->get_parameter(name + ".direct_yaw_min_vel", direct_yaw_min_vel_);
    ...
    nav2_core::GoalChecker * goal_checker) override
  {
    ...
    if (dist < direct_approach_distance_) {
      if (dist > direct_rotate_xy_tolerance_) {
        // 阶段1：xy未到位，纯平移逼近目标点
        ...
        // 阶段2：xy已到位，原地旋转到目标 yaw
        cmd.twist.linear.x = 0.0;
        cmd.twist.linear.y = 0.0;

        double current_yaw = tf2::getYaw(pose.pose.orientation);
        double target_yaw = tf2::getYaw(goal_.pose.orientation);
        double yaw_error = target_yaw - current_yaw;

        while (yaw_error > M_PI) yaw_error -= 2.0 * M_PI;
        while (yaw_error < -M_PI) yaw_error += 2.0 * M_PI;

        if (std::abs(yaw_error) < direct_yaw_tolerance_) {
          cmd.twist.angular.z = 0.0;
        } else {
          double wz = direct_yaw_kp_ * yaw_error;
          wz = std::clamp(wz, -direct_yaw_max_vel_, direct_yaw_max_vel_);
          if (wz > 0.0 && wz < direct_yaw_min_vel_) wz = direct_yaw_min_vel_;
          if (wz < 0.0 && wz > -direct_yaw_min_vel_) wz = -direct_yaw_min_vel_;
...
private:
  ...
  double direct_rotate_xy_tolerance_{0.05};
  double direct_yaw_tolerance_{0.10};
  double direct_yaw_kp_{2.0};
  double direct_yaw_max_vel_{0.8};
  double direct_yaw_min_vel_{0.3};
};
